...
| **4** | v1.1+ Post-MVP — **socials + leaderboard + full cloud sync (Scope B, in progress, see [socials_plan.md](socials_plan.md))**, health integrations, AI, web | +6 wk → +6 mo | `[~]` S0–S5 done (2026-04-30) — auth, push, pull, welcome-back hydration, cross-device convergence, contact-match, **friend graph (request/accept/decline/block/remove + username search)**; S6 leaderboard next |
...
- Status: **implementation in progress.** S0 backend, S1 auth, S2 username+phone, S3 push side, S3b pull side, S3c incremental pull (cross-device convergence), S4 contact-match flow (with country-aware lenient normalization), and S5 friend graph (`list_my_friend_graph` RPC + Friends hub + username search) are all complete as of 2026-04-30. Next: S6 leaderboard. Invite-link deep-linking deferred alongside the rest of the native URL-scheme registration. See [socials_plan.md](socials_plan.md) for the full breakdown.
