            ...
            child: Column(
              children: [
                AspectRatio(
                  aspectRatio: 2.2,
                  child: CustomPaint(
                    painter: _BodyBreakdownPainter(
                      activations: {for (final s in split) s.muscle: s.pct},
                    ),
                  ),
                ),
...
class _BodyBreakdownPainter extends CustomPainter {
  _BodyBreakdownPainter({required this.activations});
  ...
  void paint(Canvas canvas, Size size) {
    final w = size.width;
    final h = size.height;
    // Two halves: front (left) and back (right).
    _paintBody(canvas, Offset(w * 0.25, 0), w * 0.45, h, isBack: false);
    _paintBody(canvas, Offset(w * 0.75, 0), w * 0.45, h, isBack: true);
  }
  ...
  void _paintBody(
    ...
    // Head.
    canvas.drawCircle(Offset(cx, h * 0.13), w * 0.12, body);

    // Torso.
    final torso = Path()
      ..moveTo(cx - w * 0.22, h * 0.24)
      ..quadraticBezierTo(cx - w * 0.28, h * 0.32, cx - w * 0.26, h * 0.45)
      ..quadraticBezierTo(cx - w * 0.30, h * 0.55, cx - w * 0.20, h * 0.62)
      ..lineTo(cx - w * 0.13, h * 0.68)
      ..lineTo(cx + w * 0.13, h * 0.68)
      ..lineTo(cx + w * 0.20, h * 0.62)
      ..quadraticBezierTo(cx + w * 0.30, h * 0.55, cx + w * 0.26, h * 0.45)
      ..quadraticBezierTo(cx + w * 0.28, h * 0.32, cx + w * 0.22, h * 0.24)
      ..close();
    ...
    canvas.drawRRect(
      RRect.fromRectAndRadius(
        Rect.fromLTWH(cx - w * 0.42, h * 0.27, w * 0.13, h * 0.34),
    ...
    canvas.drawRRect(
      RRect.fromRectAndRadius(
        Rect.fromLTWH(cx + w * 0.30, h * 0.27, w * 0.13, h * 0.34),
        const Radius.circular(8),
      ),
      body,
    );
    ...
    canvas.drawRRect(
      RRect.fromRectAndRadius(
        Rect.fromLTWH(cx - w * 0.16, h * 0.68, w * 0.13, h * 0.30),
    ...
    canvas.drawRRect(
      RRect.fromRectAndRadius(
        Rect.fromLTWH(cx + w * 0.04, h * 0.68, w * 0.13, h * 0.30),
        const Radius.circular(6),
      ),
      body,
    );
    ...
    // Activation overlays.
    final accent = Paint()..color = AppPalette.purpleSoft.withValues(alpha: 0.85);
    final accentSoft = Paint()..color = AppPalette.purpleSoft.withValues(alpha: 0.55);

    final chest = activations['chest'] ?? 0;
    final shoulders = activations['shoulders'] ?? 0;
    final triceps = activations['triceps'] ?? 0;
    final back = activations['back'] ?? 0;
    final biceps = activations['biceps'] ?? 0;
    final core = activations['core'] ?? 0;
    final quads = activations['quads'] ?? 0;
    final glutes = activations['glutes'] ?? 0;
    final hamstrings = activations['hamstrings'] ?? 0;
    final calves = activations['calves'] ?? 0;

    Paint paintFor(int pct) =>
        pct >= 30 ? accent : (pct > 0 ? accentSoft : Paint()..color = Colors.transparent);
    ...
    if (isBack) {
      ...
      if (back > 0) {
        canvas.drawPath(
          Path()
            ..moveTo(cx - w * 0.20, h * 0.27)
            ..quadraticBezierTo(cx, h * 0.30, cx + w * 0.20, h * 0.27)
            ..quadraticBezierTo(cx + w * 0.16, h * 0.42, cx, h * 0.45)
            ..quadraticBezierTo(cx - w * 0.16, h * 0.42, cx - w * 0.20, h * 0.27)
            ..close(),
          paintFor(back),
        );
      }
      ...
      if (triceps > 0) {
        canvas.drawRRect(
          RRect.fromRectAndRadius(
            Rect.fromLTWH(cx - w * 0.40, h * 0.30, w * 0.10, h * 0.20),
            const Radius.circular(6),
          ),
          paintFor(triceps),
        );
        canvas.drawRRect(
          RRect.fromRectAndRadius(
            Rect.fromLTWH(cx + w * 0.30, h * 0.30, w * 0.10, h * 0.20),
            const Radius.circular(6),
          ),
          paintFor(triceps),
        );
      ...
      if (glutes > 0) {
        canvas.drawPath(
          Path()
            ...
            ..quadraticBezierTo(cx, h * 0.70, cx + w * 0.18, h * 0.62)
            ..quadraticBezierTo(cx + w * 0.12, h * 0.74, cx, h * 0.74)
            ..quadraticBezierTo(cx - w * 0.12, h * 0.74, cx - w * 0.18, h * 0.62)
            ..close(),
          paintFor(glutes),
        );
      }
      ...
      if (hamstrings > 0) {
        canvas.drawRRect(
          RRect.fromRectAndRadius(
            Rect.fromLTWH(cx - w * 0.14, h * 0.74, w * 0.11, h * 0.14),
        ...
        canvas.drawRRect(
          RRect.fromRectAndRadius(
            Rect.fromLTWH(cx + w * 0.04, h * 0.74, w * 0.11, h * 0.14),
            const Radius.circular(4),
          ),
          paintFor(hamstrings),
        );
      ...
      if (calves > 0) {
        canvas.drawRRect(
          RRect.fromRectAndRadius(
            Rect.fromLTWH(cx - w * 0.14, h * 0.86, w * 0.11, h * 0.10),
            const Radius.circular(4),
          ),
          paintFor(calves),
        );
        canvas.drawRRect(
          RRect.fromRectAndRadius(
            Rect.fromLTWH(cx + w * 0.04, h * 0.86, w * 0.11, h * 0.10),
            const Radius.circular(4),
          ),
          paintFor(calves),
        );
      ...
      // Front: chest, shoulders, biceps, core, quads.
      if (chest > 0) {
        canvas.drawPath(
          Path()
            ..moveTo(cx - w * 0.20, h * 0.27)
            ..quadraticBezierTo(cx, h * 0.31, cx + w * 0.20, h * 0.27)
            ..quadraticBezierTo(cx + w * 0.16, h * 0.40, cx, h * 0.42)
            ..quadraticBezierTo(cx - w * 0.16, h * 0.40, cx - w * 0.20, h * 0.27)
            ..close(),
          paintFor(chest),
        );
      }
      if (shoulders > 0) {
        canvas.drawCircle(Offset(cx - w * 0.27, h * 0.28), w * 0.10, paintFor(shoulders));
        canvas.drawCircle(Offset(cx + w * 0.27, h * 0.28), w * 0.10, paintFor(shoulders));
      }
      if (biceps > 0) {
        canvas.drawRRect(
          RRect.fromRectAndRadius(
            Rect.fromLTWH(cx - w * 0.40, h * 0.34, w * 0.11, h * 0.16),
            const Radius.circular(6),
          ),
          paintFor(biceps),
        );
        canvas.drawRRect(
          RRect.fromRectAndRadius(
            Rect.fromLTWH(cx + w * 0.29, h * 0.34, w * 0.11, h * 0.16),
            const Radius.circular(6),
          ),
          paintFor(biceps),
        );
      ...
      if (core > 0) {
        canvas.drawRRect(
          RRect.fromRectAndRadius(
            Rect.fromLTWH(cx - w * 0.13, h * 0.45, w * 0.26, h * 0.15),
            const Radius.circular(4),
          ),
          paintFor(core),
        );
      ...
      if (quads > 0) {
        canvas.drawRRect(
          RRect.fromRectAndRadius(
            Rect.fromLTWH(cx - w * 0.14, h * 0.70, w * 0.11, h * 0.18),
            const Radius.circular(4),
          ),
          paintFor(quads),
        );
        canvas.drawRRect(
          RRect.fromRectAndRadius(
            Rect.fromLTWH(cx + w * 0.04, h * 0.70, w * 0.11, h * 0.18),
            const Radius.circular(4),
          ),
          paintFor(quads),
        );
  ...
  @override
  bool shouldRepaint(covariant _BodyBreakdownPainter old) =>
      old.activations.length != activations.length ||
      activations.entries.any((e) => old.activations[e.key] != e.value);
}
