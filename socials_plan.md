...
- **S6 — Leaderboard:** ✓ core done (2026-04-30) + Hall of Glory rebuild + `monthly_xp` shipped 2026-05-01. [`LeaderboardStatsService.refresh`](lib/data/services/leaderboard_stats_service.dart) recomputes `level` / `total_xp` / `weekly_xp` / `monthly_xp` / `current_streak` / `longest_streak` / `last_active_at` from local sqflite and pushes to `public_profiles` — closes the otherwise-unwired loop where these columns sat at zero forever. Triggered from `WorkoutService.finish` / `addXp`, `StreakService.upsert`, and a defensive sweep on every `SyncEngine.drainOnce` tick. [`WorkoutService.weeklyXp`](lib/data/services/workout_service.dart) sums XP since the most recent Monday 00:00 UTC (mirrors the server's Monday rollover cron); `WorkoutService.monthlyXp` sums since the 1st of the current UTC month (mirrors migration [011_monthly_xp.sql](supabase/migrations/011_monthly_xp.sql)'s monthly cron). [`LeaderboardService.fetch(metric)`](lib/data/services/leaderboard_service.dart) reads `public_profiles` rows for `me + accepted friends` filtered + ordered server-side; RLS naturally permits the read for accepted-friend rows. [`LeaderboardScreen`](lib/screens/leaderboard/leaderboard_screen.dart) at `/leaderboard`: full Hall of Glory rebuild — gradient header, scope toggle (FRIENDS / CLASS / GLOBAL), three period tabs (This Week / Month / All Time, all real data), top-3 podium with crown bobbing animation, ranked rows with rank pill / avatar / name+username / streak chip / metric value, user's own row glow-highlighted with YOU pill, empty-state nudge to `/friends/search`, pull-to-refresh. CLASS / GLOBAL scope toggles are honestly gated as "coming soon" snackbars (no fake data shown). The leaderboard IS one of the 5 bottom tabs — reachable via the floating gold center button in [AppTabBar](lib/widgets/tab_bar.dart) (not a 5th additional tab). Streak data still surfaces inline as a per-row chip on every entry.
  - **Deferred to S6b:** Realtime subscription on `public_profiles` for "Maya passed you" toasts; local sqflite snapshot of last-known leaderboard so the tab renders offline; conditional tab visibility ("only when socials opted in") — currently always visible since v1.x.0 has no opt-out path until S7 ships.
- S7–S10: not started.
...
| `public_profiles` (username, display_name, avatar, level, total_xp, weekly_xp, monthly_xp, current_streak, longest_streak, last_active_at, phone, phone_hash) | local mirror | ✓ | Drives leaderboard reads + contact match. RLS clamps reads to friends; raw `phone` only readable by the row owner. |
...
## S1 — Auth screens ✓ done (2026-04-29) + combined Auth screen rebuild (2026-04-30)

- [x] **Combined Auth screen** — [lib/screens/auth/auth_screen.dart](lib/screens/auth/auth_screen.dart). Replaces the original separate Sign Up + Sign In + Pick Username + Phone Number screens. Single mode-toggle form collects Hero Name + email + phone (with 30-country picker, India default) + password (with strength meter), terms-of-service checkbox. Live username-availability check via `check_username_available` RPC (320ms debounce). Routes `/signup` and `/signin` both resolve to this screen.
- The standalone files `sign_up_screen.dart`, `sign_in_screen.dart`, `username_screen.dart`, and `phone_screen.dart` were deleted as part of the v1-improvements design pass — see also S2.
...
## S2 — Username + phone collection ✓ done (2026-04-29), folded into combined Auth screen (2026-04-30)

The original standalone Pick Username + Phone Number screens were collapsed into the combined Auth screen ([lib/screens/auth/auth_screen.dart](lib/screens/auth/auth_screen.dart)) as part of the v1-improvements design pass. The single Hero Name field doubles as `username` (handle) and `display_name` server-side. All the validation + RPC calls below still apply, just in one form rather than two screens.

- [x] Live username-availability check via `check_username_available` RPC (320ms debounce), inline status icon + label, `[a-z0-9_]` input filter, 30-day rename lock enforced server-side via the `username_lock` trigger.
- [x] Phone E.164 input with 30-country picker (India default), privacy disclosure card.
...
- [x] **First-sync vs ongoing-sync branching**: [`InitialSync.needed()`](lib/data/sync/initial_sync.dart) returns `true` iff authenticated AND `last_full_sync_at IS NULL`. The combined Auth screen ([lib/screens/auth/auth_screen.dart](lib/screens/auth/auth_screen.dart), sign-in mode) routes to `/welcome-back` only on first sign-in per device; subsequent sign-ins skip straight to `/home`.
...
## S6 — Leaderboard screen ✓ core done (2026-04-30); polish in S6b

- [x] Three period tabs: This Week (default, → `weekly_xp`), Month (→ `monthly_xp`), All Time (→ `total_xp`). [`LeaderboardScreen`](lib/screens/leaderboard/leaderboard_screen.dart) state-switches on the active metric; per-metric futures are lazy-loaded on first paint of that tab. Streak data still surfaces inline as a per-row chip rather than as its own tab.
- [x] Single PostgREST query per tab, filtered by friend list, sorted descending. [`LeaderboardService.fetch`](lib/data/services/leaderboard_service.dart) reads `public_profiles` rows for `me + accepted friends`, ordered server-side by the active metric. RLS naturally permits the read for accepted-friend rows.
- [x] User's own row highlighted (amber glow + "(you)" suffix). Sticky-to-top deferred to S6b.
- [x] **Stats refresh pipeline** — [`LeaderboardStatsService.refresh`](lib/data/services/leaderboard_stats_service.dart) recomputes `level` / `total_xp` / `weekly_xp` / `monthly_xp` / `current_streak` / `longest_streak` / `last_active_at` from local sqflite and pushes to `public_profiles`. Triggered from `WorkoutService.finish` / `addXp`, `StreakService.upsert`, and a defensive sweep on every `SyncEngine.drainOnce` tick. Without this, the leaderboard columns would sit at zero forever.
- [x] New bottom-tab item "Ranks" (icon `leaderboard_outlined`) added to `InAppShell` between Streak and Profile. Pill width trimmed 66 → 58 so 5 pills fit on a 375pt-wide screen.
- [ ] Realtime subscription via Supabase Realtime — listens for `public_profiles` UPDATEs on friend rows, re-sorts the list, fires "Maya passed you · #2 → #3" toast. **Deferred to S6b.**
- [ ] Local cache (last-known leaderboard snapshot in sqflite) so the tab shows yesterday's standings while offline. **Deferred to S6b.**
- [ ] Conditional tab visibility ("only when socials is opted in") — currently always visible since v1.x.0 has no opt-out path until S7 ships.
