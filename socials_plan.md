...
- **S6 — Leaderboard:** ✓ core done (2026-04-30). [`LeaderboardStatsService.refresh`](lib/data/services/leaderboard_stats_service.dart) recomputes `level` / `total_xp` / `weekly_xp` / `current_streak` / `longest_streak` / `last_active_at` from local sqflite and pushes to `public_profiles` — closes the otherwise-unwired loop where these columns sat at zero forever. Triggered from `WorkoutService.finish` / `addXp`, `StreakService.upsert`, and a defensive sweep on every `SyncEngine.drainOnce` tick. [`WorkoutService.weeklyXp`](lib/data/services/workout_service.dart) sums XP since the most recent Monday 00:00 UTC (mirrors the server's Monday rollover cron). [`LeaderboardService.fetch(metric)`](lib/data/services/leaderboard_service.dart) reads `public_profiles` rows for `me + accepted friends` filtered + ordered server-side; RLS naturally permits the read for accepted-friend rows. [`LeaderboardScreen`](lib/screens/leaderboard/leaderboard_screen.dart) at `/leaderboard`: three tabs (Weekly XP / Streak / All-time XP) with `TabBar` + `TabBarView` lazy-loading per metric, ranked rows with rank badge / avatar / handle / level / metric chip, user's own row glow-highlighted, empty-state nudge to `/friends/search`, pull-to-refresh per tab. New 5th bottom tab "Ranks" (icon `leaderboard_outlined`) inserted between Streak and Profile in [InAppShell](lib/widgets/in_app_shell.dart) + [AppTabBar](lib/widgets/tab_bar.dart). Pill width trimmed 66 → 58 so the 5 pills fit on a 375pt-wide screen (iPhone SE 3rd gen).
...
- [x] Three tabs: Weekly XP (default), Current Streak, All-time XP. [`LeaderboardScreen`](lib/screens/leaderboard/leaderboard_screen.dart) renders via `TabBar` + `TabBarView`; per-metric futures are lazy-loaded on first paint of that tab.
