...
- [ ] Contacts permission screen with privacy disclosure. Trigger: end of socials onboarding + Settings → "Find friends" button for repeat use.
- [ ] On grant: read address book → normalize to E.164 → batch-hash with server salt → POST hashes to `find_users_by_phone_hashes` RPC.
- [ ] "Friends found" screen — matched users with avatar, level, streak, `Add Friend` button.
- [ ] Empty-state ("0 found") routes prominently to the invite-link share sheet.
